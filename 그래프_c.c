...
void printpath(GRAPH* my_graph, char data1, char data2){
    if(path(my_graph, data1, data2) == 0){
...
void merge(GRAPH* my_graph, char data1, char data2){
    int i = position(my_graph, data1), j = position(my_graph, data2);
    VERTEX *vi, *vj;
    int tmp;
    char tmp_data;
    tmp = delete_edge(my_graph, data1, data2, 1);
    if(tmp == -1) return;
    vi = my_graph->vertex[i];
    vj = my_graph->vertex[j];
    while(vj->link != NULL){
        vj = vj->link;
        tmp_data = vj->data;
        delete_edge(my_graph, data1, tmp_data, 0); //합치는 edge가 이미 있으면 삭제, 없으면 -1 반환
        add_edge(my_graph, data1, tmp_data);
    }
    delete_vertex(my_graph, data2, 0);
    return;
}
...
int main(void){ //메인 함수
    ...
    printf("Q(정점1,정점2): 정점1부터 정점2로가는 경로 출력\n"); //자신만의 기능
    ...
    printf("M(정점1,정점2): 지정한 두 개의 정점을 합병 (값은 정점1의 값을 가짐)\n");
    printf("K: 그래프 비우기\n");
    printf("&: 프로그램 종료\n\n"); //각 기능별 설명
    ...
    while(1){
        ...
        for(int i=0;i<=strlen(inp);i++){
            if(inp[i] == 'G'){
                ...
                if(inp[i+1] == '(' && inp[i+5] == ')'){
                    if('A'<=inp[i+2] && 'Z'>=inp[i+2] && 'A'<=inp[i+4] && 'Z'>=inp[i+4]){
                        add_edge(&my_graph, inp[i+2], inp[i+4]);
                        i = i+5;
                    }
                }
            }
            ...
            else if(inp[i] == 'M'){ //나만의 기능 3
                if(inp[i+1] == '(' && inp[i+5] == ')'){
                    if('A'<=inp[i+2] && 'Z'>=inp[i+2] && 'A'<=inp[i+4] && 'Z'>=inp[i+4]){
                        merge(&my_graph, inp[i+2], inp[i+4]);
                    }
                }
                i = i+5;
            }
